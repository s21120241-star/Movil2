...
private fun handleLogout(context: Context, navController: NavHostController) {
    // 1. Limpiar SharedPreferences (Cookies y Matrícula)
    val prefs = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
    prefs.edit().clear().apply()

    val cookiePrefs = PreferenceManager.getDefaultSharedPreferences(context)
    cookiePrefs.edit().remove("PREF_COOKIES").apply()

    // 2. Limpiar Base de Datos (debe ser en un hilo de fondo)
    // Usamos GlobalScope solo para la operación de limpieza rápida o el scope del contexto si es posible
    kotlinx.coroutines.GlobalScope.launch(Dispatchers.IO) {
        Sicenetdatabase.getDatabase(context).clearAllTables()
